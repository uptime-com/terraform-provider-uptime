	...
	resource.Test(t, testCaseFromSteps(t, []resource.TestStep{
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckTypeSetElemAttr("uptime_check_tcp.test", "locations.*", locs[0]),
				resource.TestCheckTypeSetElemAttr("uptime_check_tcp.test", "locations.*", locs[1]),
			),
		},
		{
			ConfigDirectory: config.StaticDirectory("testdata/resource_check_tcp/locations"),
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				resource.TestCheckResourceAttr("uptime_check_tcp.test", "locations.#", "2"),
				resource.TestCheckTypeSetElemAttr("uptime_check_tcp.test", "locations.*", locs[2]),
				resource.TestCheckTypeSetElemAttr("uptime_check_tcp.test", "locations.*", locs[3]),
			),
		},
	}))
}

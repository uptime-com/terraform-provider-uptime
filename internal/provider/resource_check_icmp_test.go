	...
	resource.Test(t, testCaseFromSteps(t, []resource.TestStep{
		...
		},
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttr("uptime_check_icmp.test", "contact_groups.0", "Default"),
				resource.TestCheckResourceAttr("uptime_check_icmp.test", "contact_groups.1", "nobody"),
			),
		},
	}))
}
	...
	resource.Test(t, testCaseFromSteps(t, []resource.TestStep{
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckTypeSetElemAttr("uptime_check_icmp.test", "locations.*", locs[0]),
				resource.TestCheckTypeSetElemAttr("uptime_check_icmp.test", "locations.*", locs[1]),
			),
		},
		{
			ConfigDirectory: config.StaticDirectory("testdata/resource_check_icmp/locations"),
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				resource.TestCheckResourceAttr("uptime_check_icmp.test", "locations.#", "2"),
				resource.TestCheckTypeSetElemAttr("uptime_check_icmp.test", "locations.*", locs[2]),
				resource.TestCheckTypeSetElemAttr("uptime_check_icmp.test", "locations.*", locs[3]),
			),
		},
	}))
}

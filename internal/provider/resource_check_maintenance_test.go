	...
	resource.Test(t, testCaseFromSteps(t, []resource.TestStep{
		{
			ConfigVariables: config.Variables{
				"name":      config.StringVariable(name),
				"from_time": config.StringVariable("13:01:00"),
				"to_time":   config.StringVariable("14:59:00"),
		...
		},
		{
			ConfigVariables: config.Variables{
				"name":      config.StringVariable(name),
				"from_time": config.StringVariable("15:31:00"),
				"to_time":   config.StringVariable("23:50:00"),
				"monthday":  config.IntegerVariable(15),
			},
			ConfigDirectory: config.StaticDirectory("testdata/resource_check_maintenance/monthly"),
			Check: resource.ComposeAggregateTestCheckFunc(
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.#", "1"),
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.0.monthday", "15"),
			),
		},
	}))
}
	...
	resource.Test(t, testCaseFromSteps(t, []resource.TestStep{
		{
			ConfigVariables: config.Variables{
				...
				"to_time":           config.StringVariable("14:59:00"),
				"weekdays":          config.ListVariable(config.IntegerVariable(0), config.IntegerVariable(6)),
				"monthly_from_time": config.StringVariable("15:01:00"),
				"monthly_to_time":   config.StringVariable("16:59:00"),
				"monthday":          config.IntegerVariable(15),
				"once_start_date":   config.StringVariable(startDate),
				"once_end_date":     config.StringVariable(endDate),
			},
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.0.from_time", "13:01:00"),
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.0.to_time", "14:59:00"),
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.0.weekdays.#", "2"),
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.0.weekdays.0", "0"),
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.0.weekdays.1", "6"),
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.1.type", "MONTHLY"),
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.1.from_time", "15:01:00"),
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.1.to_time", "16:59:00"),
				resource.TestCheckResourceAttr("uptime_check_maintenance.test", "schedule.1.monthday", "15"),

	...
	resource.Test(t, testCaseFromSteps(t, []resource.TestStep{
		...
		},
		{
			...
			Check: resource.ComposeAggregateTestCheckFunc(
				...
				resource.TestCheckResourceAttr("uptime_check_ssh.test", "contact_groups.0", "Default"),
				resource.TestCheckResourceAttr("uptime_check_ssh.test", "contact_groups.1", "nobody"),
			),
		},
	}))
}
